...
def print_planet(planet_dict, parameter_dict, parameter_keywords_dict):
    for current_parameter in planet_dict:
        difference = get_parameter_difference(current_parameter, planet_dict, parameter_dict)
        difficulty = get_parameter_difficulty(current_parameter, planet_dict, parameter_dict)
        if current_parameter in ['radioactivity', 'magnetic field']:
            tabs = '\t'
        else:
            tabs = '\t\t'
        print(current_parameter.capitalize() + ': ' + str(planet_dict[current_parameter]) + ' - ' + parameter_keywords_dict[current_parameter][planet_dict[current_parameter]] + ' '+ tabs + '(difference ' + str(difference) + ', difficulty ' + str(difficulty) + ')')
    print('Total difficulty: ' + str(get_planet_difficulty(planet_dict, parameter_dict)))

def get_parameter_difference(parameter, planet_dict, parameter_dict):
    difference = abs(planet_dict[parameter] - math.floor(parameter_dict[parameter]['ideal']))
    return(difference)
...
def get_planet_difficulty(planet_dict, parameter_dict):
    ...
    for current_parameter in parameter_dict:
        ...
        weighted_difficulty += (get_parameter_difficulty(current_parameter, planet_dict, parameter_dict) * parameter_dict[current_parameter]['severity'])
...
def generate_planet(parameter_dict):
    ...
    for current_parameter in parameter_dict:
        ...
        if current_parameter == 'atmosphere':
            ...
            if planet_dict['temperature'] > 1 and planet_dict['soil'] > 1 and planet_dict['water'] > 2 and planet_dict['atmosphere'] > 2:
...
parameter_keywords_dict = {
    ...
    'size': {1: 'very small', 2: 'small', 3: 'medium', 4: 'large', 5: 'very large', 6: 'extremely large'},
    'toxicity': {1: 'none', 2: 'very low', 3: 'low', 4: 'moderate', 5: 'high', 6: 'very high'},
    'radioactivity': {1: 'none', 2: 'very low', 3: 'low', 4: 'moderate', 5: 'high', 6: 'very high'},
    'magnetic field': {1: 'none', 2: 'very weak', 3: 'weak', 4: 'moderate', 5: 'strong', 6: 'very strong'},
    'atmosphere': {1: 'very hostile', 2: 'hostile', 3: 'unpleasant', 4: 'tolerable', 5: 'pleasant', 6: 'ideal'},
...
while not purchase_phase_complete:
    inventory_dict = {'workers': 0, 'air': 0, 'water': 0, 'food': 0, 'consumer goods': 0, 'building materials': 0, 'fuel': 0}
    used_inventory = 0
    for inventory_type in inventory_dict:
        ...
        while not valid_input:
            purchased = input()
            if purchased.isdigit() and int(purchased) >= 0 and int(purchased) <= 27 - used_inventory:
                valid_input = True
    ...
    print_inventory_dict(inventory_dict)
    print('Used inventory: ' + str(used_inventory) + '/27')
    completion_confirmation = input('Enter restart to restart, or enter anything else to continue: ')
...
while inventory_dict['fuel'] > 0 and not landed:
    ...
    if planet_present:
        travel_input = input('Enter land to land here, or enter anything else to continue: ')
