...
with open("country_demographics.json") as active_file:
...
for group in country_dict:
    if group != "note":
        ...
        for country in country_dict[group]["populations"]:
            ...
            space_representation = country_dict[group]["metadata"][
...
for group in group_totals:
    ...
    weighted_percentages[group] = round(
        group_weighted_totals[group] / total_weighted_representation, 2
    )
