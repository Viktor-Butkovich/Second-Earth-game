...
global_manager.set('parameter_types', ['temperature', 'roughness', 'vegetation', 'soil', 'water'])
...
global_manager.set('valid_commands', {
    'browse': ['quit', 'save', 'new', 'print', 'delete', 'list', 'point', 'select', 'next', 'findOverlap', 'volume', 'findExpansion'],
    'terrain_view': ['quit', 'new', 'print', 'save', 'delete', 'list', 'browse', 'point', 'rename', 'select', 'next', 'findOverlap', 'volume', 'copy', 'findExpansion'],
    'point_view': ['quit', 'new', 'print', 'save', 'delete', 'list', 'browse', 'point', 'select', 'next', 'findOverlap', 'checkOverlap', 'volume', 'findExpansion']
})
...
while current_input[0] != 'quit':
    print('\nDisplay mode: ' + global_manager.get('current_display_mode').upper() + 
          '(' + 
          utility.comma_list(global_manager.get('valid_commands')[global_manager.get('current_display_mode')]) + 
          ')')
    ...
    if len(current_input) > 0:
        if current_input[0] in global_manager.get('valid_commands')[global_manager.get('current_display_mode')]:
        ...
        print('That is not a valid command in the ' + global_manager.get('current_display_mode') + ' display mode\n')

current_input = utility.extract_arguments(input('Enter "discard" to discard new changes, or enter anything else to save changes: '))
