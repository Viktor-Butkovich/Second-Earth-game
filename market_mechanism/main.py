...
class owner:
    def __init__(self, seller: bool, num_participants: int, price: float, production: int, name: str = None) -> None:
    ...
    def remove(self) -> None:
        ...
        participants.write(str(int(self.seller)) + ' ' + str(self.num_participants + self.production) + ' ' + str(self.price) + ' ' + str(self.production) + ' ' + str(self.name) + '\n')
        participants.close()
        if self.seller:
    ...
    def check_better_deal(self) -> None:
        if random.randrange(1, 7) >= 4:
            ...
            self.worsen_price()
            print(self.name + ' is now looking for a better deal for ' + str(self.price))
    ...
    def print_summary(self) -> None:
        if self.seller:
            print(self.name + ' wants to sell ' + str(self.num_participants) + ' for ' + str(self.price) + ' each', end='')
        else:
            print(self.name + ' wants to buy ' + str(self.num_participants) + ' for ' + str(self.price) + ' each', end='')
...
def get_random_participant() -> Tuple[owner, int]:
    ...
    chosen_participant = random.randrange(num_participants) # Could choose participant from 0 to 5
    for current_owner in status.owners:
...
def print_total_owners():
    ...
    print(str(len(status.sellers)) + ' sellers want to sell a total of ' + str(num_selling))
    print()
...
def conduct_round() -> bool:
    ...
    if status.buyers and status.sellers:
        ...
        if participant_1[0].seller:
            ...
            print('Participant 1: buyer ' + str(participant_1[1]) + ' of ' + participant_1[0].name + ' for ' + str(participant_1[0].price))
        if participant_2[0].seller:
            print('Participant 2: seller ' + str(participant_2[1]) + ' of ' + participant_2[0].name + ' for ' + str(participant_2[0].price))
        else:
            print('Participant 2: buyer ' + str(participant_2[1]) + ' of ' + participant_2[0].name + ' for ' + str(participant_2[0].price))
        print()
        transaction = False
        ...
        if transaction:
            quantity = 1
            while random.randrange(1, 7) >= 2 and quantity < buyer_owner.num_participants and quantity < seller_owner.num_participants:
                quantity += 1
            print(buyer_owner.name + ' bought ' + str(quantity) + ' from ' + seller_owner.name + ' for ' + str(seller_owner.price) + ' each')
                ...
                if current_owner in status.owners:
                    ...
                    if decision == 1:
                        if current_owner.looking_for_better_deal:
                            ...
                            print(current_owner.name + ' failed to find a transaction and stopped looking for a better deal (price returned to ' + str(current_owner.price) + ')')
                        ...
                        current_owner.improve_price()
                        print(current_owner.name + ' failed to find a transaction and relaxed their price to ' + str(current_owner.price))
                    else:
                        print(current_owner.name + ' failed to find a transaction but stayed in the market')
...
if input('Type reset to reset: ') == 'reset':
    ...
    test_case = 4
    if test_case == 1:
        owner(seller=True, num_participants=100, price=0.5, production=25, name='Seller1')
        owner(seller=False, num_participants=100, price=1.5, production=25, name='Buyer1')
        owner(seller=True, num_participants=100, price=0.5, production=25, name='Seller2')
        owner(seller=False, num_participants=100, price=1.5, production=25, name='Buyer2')
        owner(seller=True, num_participants=100, price=0.5, production=25, name='Seller3')
        owner(seller=False, num_participants=100, price=1.5, production=25, name='Buyer3')
        owner(seller=True, num_participants=100, price=0.5, production=25, name='Seller4')
        owner(seller=False, num_participants=100, price=1.5, production=25, name='Buyer4')
    elif test_case == 2:
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller1')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer1')
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller2')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer2')
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller3')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer3')
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller4')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer4')
    elif test_case == 3:
        owner(seller=True, num_participants=100, price=1.5, production=250, name='Seller1')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer1')
        owner(seller=True, num_participants=100, price=1.5, production=250, name='Seller2')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer2')
        owner(seller=True, num_participants=100, price=1.5, production=250, name='Seller3')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer3')
        owner(seller=True, num_participants=100, price=1.5, production=250, name='Seller4')
        owner(seller=False, num_participants=100, price=0.5, production=25, name='Buyer4')
    elif test_case == 4:
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller1')
        owner(seller=False, num_participants=100, price=0.5, production=250, name='Buyer1')
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller2')
        owner(seller=False, num_participants=100, price=0.5, production=250, name='Buyer2')
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller3')
        owner(seller=False, num_participants=100, price=0.5, production=250, name='Buyer3')
        owner(seller=True, num_participants=100, price=1.5, production=25, name='Seller4')
        owner(seller=False, num_participants=100, price=0.5, production=250, name='Buyer4')
    print_summary()
else:
    participants = open('participants.txt', 'r')
    for line in participants:
        current_list = line.strip().split()
        owner(bool(int(current_list[0])), int(current_list[1]), float(current_list[2]), int(current_list[3]), current_list[4])
    print_summary()
    participants.close()
    participants = open('participants.txt', 'w')
    participants.write('')
